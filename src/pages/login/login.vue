<template>
  <view class="login-container">
    <view class="i-ph-x-bold absolute top-4 left-4 text-2xl" @click="goBack" />
    <view class="header">
      <text class="greeting">你好，</text>
      <text class="welcome">欢迎来到旅接</text>
    </view>
    ...
    <button class="login-btn" :disabled="!phoneNumber || !verificationCode" @click="login">登录</button>
    ...
    <view class="third-party-login">
      <text class="text-sm text-gray-500">使用第三方账号登录</text>
      <view class="wechat-icon" @click="wechatLogin">
        <image src="/src/static/wechat-icon.png" class="w-10 h-10" />
      </view>
    </view>
  </view>
</template>
...
const getVerificationCode = () => {
  if (!phoneNumber.value || codeSending.value) return
